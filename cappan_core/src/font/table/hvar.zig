...
pub const HvarTable = struct {
    ...
    pub fn getAdvanceWidthDelta(self: HvarTable, glyph_id: u16, normalized_coords: []const f32) !i32 {
        return ivs.getMappedDelta(self.data, self.advance_width_mapping_offset, self.item_variation_store_offset, glyph_id, normalized_coords);
    }

    pub fn getLsbDelta(self: HvarTable, glyph_id: u16, normalized_coords: []const f32) !i32 {
        return ivs.getMappedSideBearingDelta(self.data, self.lsb_mapping_offset, self.item_variation_store_offset, glyph_id, normalized_coords);
    }
};
